...
<html lang="en">
  <head>
    ...
    <title><?= $product->props->title ?> | Шпаргалка для лучшей жизни</title>
  ...
  <link href="../product/product.bundle.css" rel="stylesheet"></head>
  <body>
...
<?php $page = $product->props->page; ?><?php
$action = false;
if ($product->props->is_enroll->value)
   ...
   $action = '';
?>
      <div class="product-preview col dn">
        <div class="product-preview__image" style="background-image: url('<?= $product->at_path('page/image') ?>'); background-position: <?= $page->props->image_pos_x ?>% <?= $page->props->image_pos_y ?>%"></div><?php if ($action) : ?><a href="../buy/?id=<?= $product->id ?>">
          <button class="button rel cup product-preview__button w100"><?= $action ?>
            <div class="button__inflation button__inflation_1 abs"></div>
            <div class="button__inflation button__inflation_2 abs"></div>
            <div class="button__inflation button__inflation_3 abs"></div>
            <div class="button__inflation button__inflation_4 abs"></div>
...
if ($product->has_path('datetime'))
   $title = "$title ({$product->props->datetime})";?>
        ...
        <div class="product-preview__tags"><?php $tags = $product->props->tags->get_children(); ?>
<?php foreach ($tags as $tag) : ?>
<?php if ($tag->type->name == 'space') continue; ?><a class="product-preview__tag product-shop__tag dib tdn cup" href="<?= '../shop/?theme=' . $tag ?>"><?= $tag ?> </a> <?php endforeach; ?>
        </div>
...
<?php if ($tag->type->name == 'space') continue; ?><a class="product-preview__tag product-shop__tag dib tdn cup" href="<?= '../shop/?theme=' . $tag ?>"><?= $tag ?> </a> <?php endforeach; ?>
  ...
  <script src="../product/product.bundle.js"></script></body>
</html>